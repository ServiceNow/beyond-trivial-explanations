...
class ResnetGroup(torch.nn.Module):
    def __init__(self, n, ni, no, stride, activation, spectral_norm, dp_prob=.3, batchnorm=None):
        ...
        in_plane = ni
        for i in range(n):
            setattr(self, "block_%d" %i, ResnetBlock(in_plane, no, stride if i==0 else 1, activation, spectral_norm, dp_prob=dp_prob, batchnorm=batchnorm))
            in_plane = no
    def forward(self, x):
...
class Resnet(torch.nn.Module):
    def __init__(self, ratio=0, depth=1, width=1, activation=F.relu, spectral_norm=False, dp_prob=.3,
        ...
        for i, (out_ch, nblocks) in enumerate(zip(self.channels, self.nblocks)):
            setattr(self, "group%d" %i, ResnetGroup(nblocks, in_ch, out_ch, stride=2, activation=activation, spectral_norm=spectral_norm, dp_prob=dp_prob, batchnorm=batchnorm))
...
class Encoder(Resnet):
    def __init__(self, z_dim, output_h, output_w, ratio=0, depth=1, width=1, dp_prob=.3, mlp_width=1, mlp_depth=1, batchnorm=torch.nn.InstanceNorm2d,
                 exp_dict=None):
        super().__init__(ratio=ratio, depth=depth, width=width, activation=lambda x: x * torch.sigmoid(x), dp_prob=dp_prob, batchnorm=batchnorm,
        exp_dict=exp_dict)
        self.z_dim = z_dim
